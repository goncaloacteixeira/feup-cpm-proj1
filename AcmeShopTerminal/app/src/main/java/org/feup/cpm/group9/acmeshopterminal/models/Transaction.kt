package org.feup.cpm.group9.acmeshopterminal.models
...
import org.feup.cpm.group9.acmeshopterminal.API_URL
...
class Transaction(
    ...
    companion object {
            ...
            val request = JsonArrayRequest(
                ...
                { response ->
                    ...
                    for (i in 0 until response.length()) {
                        ...
                        val itemsList = gson.fromJson<ArrayList<Item>>(
                            transaction.getJSONArray("items").toString(), itemsType
                        )
        ...
        fun validateToken(context: Context, token: String, callback: (Transaction?) -> Unit) {
            val queue = Volley.newRequestQueue(context)
            val url = "$API_URL/transactions/token/$token"

            val request = JsonObjectRequest(
                Request.Method.POST, url, null,
                { response ->
                    Log.i("Transaction", "validateToken: Response is: $response")
                    if (response.getString("message") == "OK") {
                        val transaction =
                            gson.fromJson(response.getJSONObject("content").toString(), Transaction::class.java)
                        val itemsType = object : TypeToken<ArrayList<Item>>() {}.type
                        val itemsList = gson.fromJson<ArrayList<Item>>(
                            response.getJSONObject("content").getJSONArray("items").toString(),
                            itemsType
                        )
