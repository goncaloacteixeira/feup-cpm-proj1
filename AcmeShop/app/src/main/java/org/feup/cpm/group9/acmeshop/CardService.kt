...
private const val AID = "F0010203040506" // AID for this applet service.
...
class CardService : HostApduService() {
  ...
  private val SELECT_APDU = BuildSelectApdu(AID)                 // the 'Select Aid x' APDU
  ...
  override fun processCommandApdu(command: ByteArray, bundle: Bundle?): ByteArray {
    return if (Arrays.equals(SELECT_APDU, command)) {
      val accountBytes = TR_TOKEN.toByteArray()
